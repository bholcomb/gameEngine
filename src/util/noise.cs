...
namespace Util
{
   ...
   public class SimplexNoise
   {
      ...
      public SimplexNoise(Int32 seed=0) 
      {
         setSeed(seed);
      }
      ...
      public void setSeed(Int32 seed)
      {
         ...
         //setup the default permutations
         ...
         for (int i = 0; i < 256; i++)
         {
            ...
            //put it in the permutations table
            p[i] = newP[index];
      ...
      #region simplex noise functions
      // 2D simplex noise
      ...
      #region fractal brownian motion generators
      //fractal brownian motion to combine noise functions (each called an octive) at certain frequencies
      public double fbm2(double xin, double yin, int octaves, double gridSize=512.0, double lacunarity=2.0, double gain=0.5)
      {
         double total = 0.0f;
         double frequency = 1.0f / (float)gridSize;
         double amplitude = gain;

         for (int i = 0; i < octaves; ++i)
         {
            total += noise(xin * frequency, yin * frequency) * amplitude;
      ...
      public double fbm3(double xin, double yin, double zin, int octaves, double gridSize=512.0, double lacunarity = 2.0, double gain = 0.5)
      {
         double total = 0.0f;
         double frequency = 1.0f / (float)gridSize;
         double amplitude = gain;
         ...
         for (int i = 0; i < octaves; ++i)
         {
            total += noise(xin * frequency, yin * frequency, zin * frequency) * amplitude;
            frequency *= lacunarity;
            amplitude *= gain;
         }
      ...
      public double fbm4(double xin, double yin, double zin, double win, int octaves, double gridSize=512.0, double lacunarity = 2.0, double gain = 0.5)
      {
         double total = 0.0f;
         double frequency = 1.0f / (float)gridSize;
         double amplitude = gain;
         ...
         for (int i = 0; i < octaves; ++i)
         {
            total += noise(xin * frequency, yin * frequency, zin * frequency, win * frequency) * amplitude;
            frequency *= lacunarity;
            amplitude *= gain;
         }

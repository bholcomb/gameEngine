...
namespace Noise
{
    public sealed class RotateDomain : ModuleBase
    {
        private readonly Double[,] rotationMatrix = new Double[3, 3];

        public RotateDomain(ModuleBase source, Double x, Double y, Double z, Double angle)
        {
            this.Source = source;
            this.X = new Constant(x);
            this.Y = new Constant(y);
            this.Z = new Constant(z);
            this.Angle = new Constant(angle);
        }

        public ModuleBase Source { get; set; }

        public ModuleBase X { get; set; }

        public ModuleBase Y { get; set; }

        public ModuleBase Z { get; set; }

        public ModuleBase Angle { get; set; }

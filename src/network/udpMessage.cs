...
namespace Network
{
   public class UdpMessageServer
   {
      ...
      public UdpMessageServer(int port)
      {
         ...
         myListenThread.Name = "UDP Message Listener thread";
   ...
   public class UdpMessageClient
   {
      ...
      public UdpMessageClient(String serveraddress, int port)
      {
         myServerAddress = serveraddress;
         myPort = port;
         myClientThread = new Thread(new ThreadStart(connectToServer));
         myClientThread.Name = "UDP Message Client thread";
            ...
            while (myTimeToStop == false)
            {
               try
               {
                  ...
                  if (clientStream.DataAvailable == true)
                  {
                     ...
                     if (msgSize > (1024 * 64)) // a reasonable number, 64Kb
                        throw new Exception("Message received is too big");

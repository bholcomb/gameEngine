...
namespace Sim
{
   ...
   public class SpatialView : EntityDatabaseView, IDisposable
   {
         ...
         : base(db)
      {
         ...
         Application.eventManager.addListener(handleAttributeUpdate, "entity.attribute.position");
      }
      ...
      static SpatialView()
      {
         ...
         Application.onPostFrame += new postFrame(SpatialView_onPostFrame);
      }
      ...
      public void Dispose()
      {
         Application.eventManager.removeListener(handleAttributeUpdate, "entity.attribute.position");
      }
      ...
      public void update(UInt64 id)
      {
         ...
         {
            if (convertPosition(ref el, el.myObject.state.attribute<Vector3>(Attributes.Position).value()) == true)
            {
               theOctree.update(el);
            }
         }
      ...
      {
         if (e.state.hasAttribute(Attributes.Position) ==true)
         {
            return true;
         }
         return false;
      ...
      protected override void checkEntityAdd(Entity e)
      {
         ...
         {
            Attribute<Vector3> pos = e.state.attribute<Vector3>(Attributes.Position);
            if (e != null)
            {
